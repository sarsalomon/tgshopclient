...
import { fetchOrders, getCategory, getProduct, getUser, deleteOrder } from '../../http/orderApi';
...
const Orders = observer(() => {
    ...
    let newsd
    let getInfo = async function(item){
        const getC = await getCategory(item.categoryId)
        const getP =  getProduct(item.productId)
        const getU =  getUser(item.userId)
        newsd = getC

    }
    
    for(let j=0;j<currentItem.length; j++){
        getInfo(currentItem[j]) 
    }
